...
type llexpr =
  ...
  | LTake of llexpr * int (** A constructor for getting rid of tuples as a matchmaking pattern.
      By index in the list. *)

(** Typed binding type *)
type llbinding =
  | LLet of typed_id * typed_id list * llexpr (** Typed expression for let declaration *)
  | LLetRec of typed_id * typed_id list * llexpr 
